...
function Modal({ children, show, header, footer, onClick }) {
  const modal = (
    <React.Fragment>
      {show && (
        <div
      ...
      <div
        className={`${
          show ? "block translate-y-0 transition-all duration-300" : " -translate-y-full transition-all duration-300"
        } bg-transparent w-full flex items-center justify-center z-50 h-full fixed p-10`}
      >
        {" "}
        <div className="max-w-lg bg-white container rounded-md h-[400px] relative shadow-lg">
          ...
          <footer className="bg-white  w-full p-4  absolute bottom-0 rounded-b-md border-t-slate-300 shadow-inner">
            <span>{footer}</span>
          </footer>
        </div>
      </div>
